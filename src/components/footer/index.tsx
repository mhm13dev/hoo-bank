                ...
                <ul className="list-none mt-4">
                  {footerLink.links.map((link) => {
                    return (
                      <li
                        key={link.name}
                        className="font-poppins font-normal text-base leading-[24px] text-dimWhite hover:text-secondary cursor-pointer mb-4 last-of-type:mb-0"
                      >
                        {link.name}
                      </li>
                    );
          ...
          {socialMedia.map((social) => {
            return (
              <img
                ...
                src={social.icon}
                alt={social.id}
                className="w-[21px] h-[21px] object-contain mr-6 last-of-type:mr-0 cursor-pointer"
              />
            );
          })}
        </div>
